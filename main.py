...
from database import init_db, init_last_seen, DB_NAME
...
async def email_check_loop(application):
    await asyncio.sleep(10)  # first delay
    while True:
        try:
            await check_new_emails(application)
        except Exception as e:
            print(f"Email check error: {e}")
        await asyncio.sleep(30)  # repeat interval


async def post_init(application):
    asyncio.create_task(email_check_loop(application))
...
def main():
    ...
    connect_handler = ConversationHandler(
        entry_points=[CallbackQueryHandler(connect_start, pattern="connect_start")],
        states={
    ...
    send_handler = ConversationHandler(
        ...
        states={
            ...
            SEND_SUBJECT: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, send_subject),
